...
class StaffingAutonomousAgent:
                ...
                "card": {
                    ...
                    "sections": [{
                        "widgets": [
                            ...
                            { "buttonList": { "buttons": [{
                                "text": bouton_texte,
                                "onClick": { "openLink": { "url": "https://script.google.com/macros/s/AKfycbxNx6y8EZRE0lVinOVqQdA8BoD0PLTzxzmlnTNU_GVyybn0-68nmtDEu7Es2-kFCqT5/exec" } }
                            }]}}
                        ]
                    }]
                }
    ...
    def analyser_et_decider(self):
        ...
        if caisse_count < 3 and facteur_stress > 1.1:
            ...
            self.envoyer_push_interactif("Alerte Saturation", alerte_msg)
        ...
        for rayon, perfs in self.perf_rayons.items():
            if perfs["statut"] == "SOUS-PERFORMANCE" and rayon not in rayons_couverts:
                actions_correctives.append({
                    ...
                    "priorite": "CRITIQUE",
                    "message": f"📉 Sous-performance {rayon} sans couverture prévue.",
    ...
    def generer_briefing_matin(self):
        msg = "🌅 *Bonjour Pierre !*\n\nVoici ton briefing d'anticipation pour aujourd'hui :\n🎯 *Objectif Taux ID :* 85%\n⚠️ *Point de vigilance :* Pic attendu à 15h."
        self.envoyer_push_interactif("Briefing Nostradamus", msg)
